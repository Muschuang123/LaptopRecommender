...
本项目适合用于线下门店的电脑选择与导购辅助。顾客到店后通常会给出预算、用途、品牌偏好、便携性、游戏或生产力需求等零散信息；店员可以先通过筛选页快速缩小候选范围，再使用对话式推荐让大模型追问关键信息并调用本地数据库工具生成推荐。候选机型可以加入购物车形成对比清单，并在需要时导出带备注和导出时间的 JSON 文件，便于门店留档、复盘或交接给其他导购继续跟进。

### 项目亮点

- 通过合法爬虫采集 ZOL 榜单和允许访问的参数页，清洗为结构化数据，并支持安全地自动更新 MySQL 数据库。
- 接入 DeepSeek 大模型，模型通过受控工具调用本地数据库进行检索和详情查询，而不是只依赖模型记忆生成推荐。
- 覆盖品牌、用途、CPU/GPU、价格、内存、硬盘、屏幕、重量等关键购机条件，适合门店快速完成多维筛选。
- 推荐结果包含机型详情、推荐理由和追问问题，能把顾客需求、数据库候选和导购解释串联起来。
- 购物车可沉淀候选清单，支持批量管理和导出 JSON，方便门店做方案留存、客户回访和内部交接。
